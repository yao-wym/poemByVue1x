    ...
    $.fn.date = function (options, Ycallback, Ncallback) {
        ...
        function refreshTime() {
            ...
            if (initH > 12) {    //判断当前时间是上午还是下午
                SecondScroll.scrollTo(0, initD * 60 - 60, 100);   //显示“下午”
                initH = initH - 12 - 1;
            }
            HourScroll.scrollTo(0, initH * 60, 100);
            MinuteScroll.scrollTo(0, initI * 60, 100);
        ...
        function init_IScroll_datetime() {
            HourScroll = new IScroll("#Hourwrapper", {
                snap       : "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexH = Math.round((this.y / 60) * (-1)) + 1;
                    HourScroll.refresh();
                }
            })
            ...
            MinuteScroll = new IScroll("#Minutewrapper", {
                snap       : "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexI = Math.round((this.y / 60) * (-1)) + 1;
                    HourScroll.refresh();
                }
            })
